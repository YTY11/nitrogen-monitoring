...
export function updataN2MonitorState(floor,line,machineId,flag){
 ...
 export function updataN2SpecValue(floor,line,machineId,lowLimit,highLimit){
  console.log(floor,line,machineId,lowLimit,highLimit);
   return request({
     url:`/Default.aspx?Action=UpdateN2SpecValue&Url={"Floor":"`+ floor +`","LineName":"`+ line +`","MachineId":"`+ machineId +`","LowLimit":"`+ lowLimit +`",HighLimit:"`+ highLimit +`"}`,
    //  params:{
