  ...
  <div class="info">
    ...
    <div class="content-container">
      <table border="0" cellspacing="0">
        ...
        <tbody>
          <tr v-for="(item,index) in showExceptionList" :key="index">
        <td>{{item.Floor}}</td>
        <td>{{item.LineName }}</td>
        ...
        <td>
          <button class="typebut" @click="typebut(item.Id)" v-if="item.State==0">
            <span class="textblink">待處理</span>
          </button>
          <button class="typebut2" @click="typebut2(item.Id)" v-if="item.State==1">處理中</button>
          <button class="typebut3" v-if="item.State==2">已處理</button>
