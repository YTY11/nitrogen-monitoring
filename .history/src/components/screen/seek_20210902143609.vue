<template>
  <div class="seek">
    ...
    <div class="seek-div" @click="showFloor">
      <input type="text" readonly="" placeholder="楼层"  />
      <ul v-show="isShowFloor">
        <li  v-for="(floor,id) in Floors" :key="id" @click="getFloor(floor)">item</li>
      </ul>
    </div>
    <div class="seek-div" @click="showLine">
      <input type="text" readonly="" placeholder="线体"  />
      <ul v-show="isShowLint">
        <li  v-for="(item,id) in Lines" :key="id">item</li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    showXian() {
      this.isShowXian = !this.isShowXian;
    },
