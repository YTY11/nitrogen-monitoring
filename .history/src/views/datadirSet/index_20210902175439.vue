...
import { getN2SpecData,updataN2MonitorState,updataN2SpecValue } from "@/api/datadirSet/datadirSet";
export default {
  name: "DatadirSet",
  components: {
    N2Header,
  ...
  methods: {
    ...
    updataN2MonitorState(floor,line,machineId,flag){
      updataN2MonitorState(floor,line,machineId,flag).then(res=>{
         if(res.Status == 0){
            console.log("修改成功")
          }
      }).catch(err=>{
     ...
     updataN2SpecValue(floor,line,machineId,lowLimit,highLimit){
       updataN2SpecValue(floor,line,machineId,lowLimit,highLimit).then(res=>{
if(res.Status == 0){
            console.log("修改成功")
          }
       }).catch(err=>{
