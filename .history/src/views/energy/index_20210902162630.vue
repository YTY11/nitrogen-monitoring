    ...
    <el-main class="n2-electronic-main">
      ...
      <Underly class="underly-center" title="楼层氮气消耗排名">
        <MonochromeBar
          ...
          :xDatas="n2EnergyFloors"
          :yDatas="n2EnergyConsume"
          class="bar-size2"
        />
        <MonochromeBarColors style="margin-top: 0.08rem" :xDatas="n2EnergyLineNames" :yDatas="n2EnergyLineConsume" class="bar-size2" />
      </Underly>
      <Underly class="underly-footer" title="线体单Psc能耗">
        <MonochromeLine :lintNames="lineDanConsume" class="line-size" />
      </Underly>
    </el-main>
  </el-container>
</template>
...
export default {
  ...
  created() {
    ...
    this.getByDayPcsData(this.floor, this.floor);
    this.getByWeekPcsData(this.floor,this.floor);
    this.getByMonthPcsData(this.floor,this.floor);
    this.getFloorN2EnergyData()
    this.getLineN2EnergyData("D061F")
    this.getLineDanEnergyData("D061F")
  },
  methods: {
    //楼层数据
    getEnergyFloors() {
    ...
    getFloorN2EnergyData(){
      getFloorN2EnergyData().then(res=>{
         if (res.Status == 0) {
            this.n2EnergyFloors = res.Data.Floor
            this.n2EnergyConsume = res.Data.Consume
          }
      }).catch(err=>{
        console.log(err);
      })
    ...
    //D06-1F各线体氮气消耗数据
    getLineN2EnergyData(floor){
      getLineN2EnergyData(floor).then(res=>{
        if (res.Status == 0) {
          this.n2EnergyLineNames = res.Data.LineName
          this.n2EnergyLineConsume = res.Data.Consume
        }
      }).catch(err=>{
        console.log(err);
      })
    ...
    //线体单Pcs消耗数据
    getLineDanEnergyData(floor){
      getLineDanEnergyData(floor).then(res=>{
        if(res.Status == 0){
          console.log(res);
          this.lineDanConsume = res.Data.Consume
        }
      }).catch(err=>{
        console.log(err);
      })
