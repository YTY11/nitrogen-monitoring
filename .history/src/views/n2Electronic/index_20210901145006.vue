<template>
  <el-container>
    ...
    <el-main class="n2-electronic-main">
      <MonitoringTotal :detailMessage="detailMessage" />
      <Underly title="异常楼层排行">
        <MonochromeBar class="err-floor" />
        <UnderlyTable/>
      </Underly>
  ...
  methods: {
    ...
    jianKongAllData(){
      jianKongAllData().then(resolve=>{
        if(resolve.Status == 0){
          this.detailMessage = resolve.Data.detailMessage
          console.log(this.detailMessage);
          this.detailMessageItemList.push(this.detailMessage.AllNormal,this.detailMessage.AllAbnormal,this.detailMessage.AllNormal)
        }
      }).catch(err=>{
        console.log(err);
      })
