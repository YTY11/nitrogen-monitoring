<template>
  <div class="n2-error">
    ...
    <div class="content-bottom">
      <Info :exceptionList="showExceptionList"/>
    </div>
  </div>
</template>
      ...
      getErrorData(floor, line)
        .then((res) => {
          if (res.Status == 0) {
            ...
            //H級優先展示
            let hList = this.showExceptionList.filter(item => item.Priority == 'H')
            let mList = this.showExceptionList.filter(item => item.Priority == 'M')
...
upData(id,EndTime,RepairContent,State).then(res=>{
  console.log(res);
}).catch
},
    // 查询数据
