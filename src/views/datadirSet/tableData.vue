    ...
    <table>
      ...
      <tbody>
        ...
        <td class="edit" v-else>
          <input type='number' name='editname' class='eidt-input' v-model="item.newLowLimit" >
        </td>
        <td class="edit" v-if="item.msg=='修改'">{{item.HighLimit}}</td>
        <td class="edit" v-else>
          <input type='number' name='editname' class='eidt-input' v-model="item.newHighLimit"  >
        </td>
        <td>
          <span  type="button" class="layui-btn layui-btn-sm layui-btn-normal" @click="change(index,item.Floor,item.LineName,item.MachineId,item.msg)" style="width: 60px;">{{item.msg}}</span>
        </td>
...
export default {
  ...
  methods:{
    ...
    change(index,floor,line,machineId,msg) {
      this.$emit("setData",index,floor,line,machineId,msg)
    },

    //判断是否监控
    changeChecked(floor,line,machineId,flag){
      this.$emit("changeChecked",floor,line,machineId,flag)
    },
  }
};
</script>
